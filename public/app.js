...
// PANİK BUTONU KONTROLCÜSÜ
function checkLockdown(data) {
...
async function init() {
    try {
        ...
        if (checkLockdown(data)) return; // KİLİT VARSA SİSTEMİ DURDUR
        ...
        if (data.success) {
            ...
            if (data.isBoostActive) {
                ...
                document.getElementById('boost-multiplier').innerText = data.boostMultiplier;
            } else {
    ...
    finally { setTimeout(() => { const loader = document.getElementById('loader'); if(loader) loader.style.opacity = '0'; setTimeout(()=> loader.style.display='none',500) }, 800); }
}
...
function updateUI() {
    ...
    const now = new Date();
    const diff1 = (now - new Date(user.lastLootbox1 || 0)) / (1000 * 60 * 60); const diff2 = (now - new Date(user.lastLootbox2 || 0)) / (1000 * 60 * 60); const diff3 = (now - new Date(user.lastLootbox3 || 0)) / (1000 * 60 * 60);
...
function showPage(p, el) { 
    triggerHaptic('light'); document.querySelectorAll('.page').forEach(page => page.classList.remove('active')); document.querySelectorAll('.dock-item').forEach(nav => nav.classList.remove('active')); document.getElementById(`page-${p}`).classList.add('active'); if(el) el.classList.add('active'); if(p === 'network') loadLeaderboard();
}
...
async function joinProject(projectId, url) {
    ...
    if (btn.innerText.includes("KATIL")) {
        triggerHaptic('light'); try { tg.openLink(url); } catch(e) { window.open(url, '_blank'); }
        btn.innerText = "🔄 KONTROL ET"; btn.style.background = "#f59e0b"; 
...
async function postAirdrop() {
    const btn = document.getElementById('btn-post-ad'); if(btn && btn.disabled) return;
    ...
    try {
        ...
        else { triggerHaptic('error'); tg.showAlert(data.message || "Yetersiz GEP"); }
    } catch(e) { tg.showAlert("Bağlantı hatası oluştu!"); } finally { if(btn) { btn.disabled = false; btn.style.opacity = "1"; btn.innerText = "YAYINLA"; } }
}
...
function renderTasks() { 
    const cont = document.getElementById('task-list'); if(!cont) return; 
...
async function completeTask(e, tid, link) { 
    const btn = document.getElementById(`task-btn-${tid}`); if (!btn) return;
    if (btn.innerText.includes("GİT")) { triggerHaptic('light'); try { tg.openLink(link); } catch(err) { window.open(link, '_blank'); } btn.innerText = "🔄 KONTROL ET"; btn.style.background = "#f59e0b"; btn.style.color = "#fff"; btn.classList.remove('btn-outline'); } 
    else if (btn.innerText.includes("KONTROL ET")) {
...
function openGame(game) { 
    ...
    if(game === 'gepcoz') { 
        document.getElementById('gepcoz-result').innerText = "Terminal Hazır. Şifreyi Çöz."; document.getElementById('gepcoz-result').style.color = "#fff"; document.getElementById('hcaptcha-widget').innerHTML = ''; 
        if(window.hcaptcha) { hcaptchaWidgetId = hcaptcha.render('hcaptcha-widget', { 'sitekey': '10b4d376-fcd6-43a0-a0ac-6388f0c418a4', 'theme': 'dark', 'callback': function(token) { verifyGepcoz(token); } }); } else { document.getElementById('gepcoz-result').innerText = "Güvenlik ağı yüklenemedi."; }
    }
}

async function verifyGepcoz(token) {
    const resText = document.getElementById('gepcoz-result'); resText.innerText = "Doğrulanıyor..."; resText.style.color = "var(--cyan)";
    try {
        const res = await fetch(`${API}/api/arcade/gepcoz`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ token, initData: tg.initData }) });
        const data = await res.json(); if (checkLockdown(data)) return;
        if (data.success) {
            user.points = data.points; updateUI(); triggerHaptic('success'); resText.innerText = `✅ AĞ KIRILDI! +${data.reward} GEP`; resText.style.color = "var(--success)"; spawnFloatingText(null, `+${data.reward} GEP`, "var(--success)");
            setTimeout(() => { if(hcaptchaWidgetId !== undefined && window.hcaptcha) hcaptcha.reset(hcaptchaWidgetId); resText.innerText = "Yeni şifre bekleniyor..."; resText.style.color = "#fff"; }, 3000);
        } else {
            triggerHaptic('error'); resText.innerText = `❌ HATA: ${data.message || "Bilinmiyor"}`; resText.style.color = "var(--danger)";
            setTimeout(() => { if(hcaptchaWidgetId !== undefined && window.hcaptcha) hcaptcha.reset(hcaptchaWidgetId); }, 2000);
        }
    } catch(e) { resText.innerText = "❌ Bağlantı koptu."; resText.style.color = "var(--danger)"; }
}

function closeGame(game) { triggerHaptic('light'); document.getElementById(`modal-${game}`).style.display = 'none'; if(game === 'gepcoz') document.getElementById('hcaptcha-widget').innerHTML = ''; }

async function playZarzara() {
    const amount = parseInt(document.getElementById('zarzara-bet').value);
    if (!amount || isNaN(amount) || amount < 100) return tg.showAlert("Min bahis 100 GEP.");
    const btn = document.getElementById('btn-do-zarzara'); const display = document.getElementById('zarzara-display'); const resultText = document.getElementById('zarzara-result');
    btn.disabled = true; resultText.innerText = "Zar atılıyor..."; resultText.style.color = "#fff"; display.classList.add('shake-box'); triggerHaptic('heavy');
    try {
        ...
        const data = await res.json(); if (checkLockdown(data)) return;
        if (!data.success) { display.classList.remove('shake-box'); resultText.innerText = "Hata!"; tg.showAlert(data.message); btn.disabled = false; return; }
        setTimeout(() => {
            display.classList.remove('shake-box'); user.points = data.points; updateUI(); const diceEmojis = { 1: "⚀", 2: "⚁", 3: "⚂", 4: "⚃", 5: "⚄", 6: "⚅" }; display.innerText = diceEmojis[data.diceValue] || "🎲";
            if (data.winAmount > 0) { resultText.innerText = `KAZANDIN! (+${data.winAmount} GEP)`; resultText.style.color = "var(--success)"; triggerHaptic('success'); spawnFloatingText(null, `+${data.winAmount} GEP`, "var(--success)"); } 
            else { resultText.innerText = "KAYBETTİN!"; resultText.style.color = "var(--danger)"; triggerHaptic('error'); }
            btn.disabled = false;
        }, 1500);
    } catch(e) { display.classList.remove('shake-box'); resultText.innerText = "Bağlantı Hatası!"; btn.disabled = false; }
}
...
async function playSpin() { 
    ...
    wheel.style.transition = 'none'; let currentRotation = parseFloat(wheel.getAttribute('data-rotation') || 0); let normalized = currentRotation % 360; wheel.style.transform = `rotate(${normalized}deg)`; void wheel.offsetWidth; wheel.style.transition = 'transform 4s cubic-bezier(0.15, 0.9, 0.15, 1)';
    try {
        ...
        const prizeAngles = { 0: 36, 2500: 108, 5000: 180, 10000: 252, 50000: 324 }; const targetAngle = prizeAngles[data.prize]; const offset = 360 - targetAngle; const jitter = Math.floor(Math.random() * 40) - 20; const newRotation = currentRotation + (360 * 5) + offset + jitter - normalized;
        wheel.style.transform = `rotate(${newRotation}deg)`; wheel.setAttribute('data-rotation', newRotation);
        setTimeout(() => { 
            user.points = data.points; updateUI(); resText.innerText = data.msg; 
            if(data.prize > 0) { resText.style.color = "var(--success)"; triggerHaptic('success'); spawnFloatingText(null, `+${data.prize} GEP`, "var(--success)"); } else { resText.style.color = "var(--danger)"; triggerHaptic('error'); } 
            btn.disabled = false; 
        }, 4000); 
...
async function playPredict(guess) {
    document.getElementById('predict-buttons').style.display = 'none'; document.getElementById('predict-loading').style.display = 'flex'; document.getElementById('predict-price').innerText = "Fiyat Alınıyor..."; triggerHaptic('medium');
    ...
    const dataStart = await resStart.json(); if (checkLockdown(dataStart)) return;
    if(!dataStart.success) { document.getElementById('predict-loading').style.display = 'none'; document.getElementById('predict-buttons').style.display = 'flex'; document.getElementById('predict-price').innerText = "Tekrar Dene"; return tg.showAlert(dataStart.message || "Hata oluştu."); }
    user.points = dataStart.points; updateUI(); let timeLeft = 10; document.getElementById('predict-price').innerHTML = `<span style="color:var(--cyan); font-size:24px;">Giriş: $${dataStart.price1}</span><br>Sonuç bekleniyor: ${timeLeft}s`;
    const timer = setInterval(() => { timeLeft--; document.getElementById('predict-price').innerHTML = `<span style="color:var(--cyan); font-size:24px;">Giriş: $${dataStart.price1}</span><br>Sonuç bekleniyor: ${timeLeft}s`; }, 1000);
    setTimeout(async () => {
        clearInterval(timer); document.getElementById('predict-price').innerText = "Sonuç Analiz Ediliyor...";
        ...
        const dataResult = await resResult.json(); if (checkLockdown(dataResult)) return;
        ...
        if(dataResult.success) {
            ...
            if(dataResult.won) { triggerHaptic('success'); spawnFloatingText(null, "+ Kazanıldı", "var(--success)"); } else { triggerHaptic('error'); }
        } else { if(dataResult.points) { user.points = dataResult.points; updateUI(); } document.getElementById('predict-price').innerText = "HATA!"; tg.showAlert(dataResult.message); }
...
async function playLootbox(boxType) { 
    const icons = { 1: "📦", 2: "🎁", 3: "💎" }; const colors = { 1: "var(--text-main)", 2: "#3b82f6", 3: "#f59e0b" }; 
    document.getElementById('lootbox-selection').style.display = 'none'; const animDiv = document.getElementById('lootbox-animation'); const boxIcon = document.getElementById('lootbox-icon'); const resText = document.getElementById('lootbox-result'); const backBtn = document.getElementById('btn-back-lootbox');
    animDiv.style.display = 'flex'; boxIcon.innerText = icons[boxType]; boxIcon.style.filter = `drop-shadow(0 0 20px ${colors[boxType]})`; resText.style.color = colors[boxType]; resText.innerText = "Kilit Çözülüyor..."; boxIcon.classList.add('shake-box'); triggerHaptic('heavy');
    ...
    setTimeout(() => {
        ...
        if(data.success) {
            ...
            updateUI(); resText.innerText = `KAZANDIN: +${data.prize}`; boxIcon.innerText = "✨";
            if(data.prize > 0) { resText.style.color = "var(--success)"; triggerHaptic('success'); spawnFloatingText(null, `+${data.prize} GEP`, "var(--success)"); } else { resText.style.color = "var(--danger)"; boxIcon.innerText = "🗑️"; triggerHaptic('error'); }
