...
// --- GEPÇÖZ HCAPTCHA SCRIPT YÜKLEME ---
...
// OYUN AÇMA FONKSİYONU VE GEPÇÖZ ENTEGRASYONU
...
function openGame(game) { 
    ...
    if(game === 'gepcoz') { 
        ...
        document.getElementById('gepcoz-result').style.color = "#fff";
        ...
        if(window.hcaptcha) {
            ...
            document.getElementById('gepcoz-result').innerText = "Güvenlik ağı yüklenemedi. Lütfen tekrar deneyin.";
        }
    }
}
...
async function verifyGepcoz(token) {
    ...
    try {
        ...
        if (data.success) {
            ...
            resText.innerText = `✅ AĞ KIRILDI! +${data.reward} GEP`;
            resText.style.color = "var(--success)";
            spawnFloatingText(null, `+${data.reward} GEP`, "var(--success)");
            
            setTimeout(() => { 
                if(hcaptchaWidgetId !== undefined && window.hcaptcha) {
                    hcaptcha.reset(hcaptchaWidgetId);
                }
                resText.innerText = "Yeni şifre bekleniyor...";
                resText.style.color = "#fff";
            }, 3000);
        } else {
            triggerHaptic('error');
            resText.innerText = `❌ HATA: ${data.message || "Bilinmiyor"}`;
        ...
        resText.innerText = "❌ Bağlantı koptu.";
        resText.style.color = "var(--danger)";
    }
}
