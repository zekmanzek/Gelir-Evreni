...
async function init() {
    try {
        ...
        if (data.success) {
            user = data.user;
            // Dinamik Sistem Ayarlarını Kaydet
            sysConfig.miningDuration = data.miningDuration || 240;
            sysConfig.announcementSpeed = data.announcementSpeed || 15;
            sysConfig.isSpinActive = data.isSpinActive;
            sysConfig.isCrashActive = data.isCrashActive;
...
function openGame(game) { 
    // 🔥 ŞALTER KONTROLÜ 🔥
    if(game === 'spin' && !sysConfig.isSpinActive) return tg.showAlert("⚠️ Çark sistemi şu an bakımda.");
    if(game === 'crash' && !sysConfig.isCrashActive) return tg.showAlert("⚠️ Roket pisti şu an bakımda.");
...
async function playSpin() { 
    ...
    const resText = document.getElementById('spin-result'); 
    btn.disabled = true; 
    resText.style.color = "#fff"; 
    ...
    try { 
        ...
        setTimeout(() => { 
            ...
            updateUI(); 
            resText.innerText = data.msg; 
            if(data.prize > 0) { 
                resText.style.color = "var(--success)"; 
                triggerHaptic('success'); 
                showGameReaction(`+${data.prize} GEP`, "var(--success)", "modal-spin"); 
            } else { 
                resText.style.color = "var(--danger)"; 
                triggerHaptic('error'); 
                showGameReaction(`BOŞ!`, "var(--danger)", "modal-spin"); 
            } 
            btn.disabled = false; 
        }, 4000); 
    } catch(e) { 
...
async function startMining(event) { triggerHaptic('heavy'); const btn = document.getElementById('btn-mine'); if(btn.disabled) return; btn.disabled = true; const res = await fetch(`${API}/api/mine`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ initData: tg.initData }) }); const data = await res.json(); if (checkLockdown(data)) return; if(data.success) { user.points = data.points; user.lastMining = new Date().toISOString(); updateUI(); triggerHaptic('success'); spawnFloatingText(event, "+" + data.reward, "var(--cyan)"); runTimer(sysConfig.miningDuration * 60 * 1000); // 🔥 DİNAMİK SÜRE 🔥
    } else { triggerHaptic('error'); if(event.target && event.target.id === 'btn-mine') tg.showAlert(data.message); btn.disabled = false; } }
function checkMiningTimer() { if (!user.lastMining || new Date(user.lastMining).getTime() === 0) return; const diff = new Date().getTime() - new Date(user.lastMining).getTime(); if (diff < sysConfig.miningDuration * 60 * 1000) runTimer((sysConfig.miningDuration * 60 * 1000) - diff); } // 🔥 DİNAMİK SÜRE 🔥
...
function renderAnnouncements(annList) { 
    ...
    if (annList && annList.length > 0) { 
        ...
        // 🔥 DİNAMİK HIZ AYARI 🔥
        annScroll.style.animationDuration = `${sysConfig.announcementSpeed}s`;
    } else { 
