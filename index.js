...
mongoose.connect(MONGODB_URI)
    .then(() => console.log("✅ Gelir Evreni v5.0 - Otonom Bot Aktif"))
...
const Settings = mongoose.model('Settings', new mongoose.Schema({
    ...
    mainGroupId: { type: String, default: "" } // YENİ: Botun atandığı ana grubun ID'si
}));
...
// ==========================================
// 🤖 OTONOM MESAJ SİSTEMİ (GÜNAYDIN / İYİ GECELER)
// ==========================================

const morningMsgs = [
    "☀️ Günaydın Siber Ağ! Madenleri toplamayı unutmayın, kazanç beklemez.", 
    "🌅 Günaydın! Yeni bir gün, yeni GEP'ler... Cüzdanlar dolsun!", 
    "☕ Günaydın Gelir Evreni ailesi! Güne bir reklam bileti alarak başlamaya ne dersiniz?",
    "🌞 Herkese günaydın. Bugün BTC yönü sizce ne olur? Kripto Kahini'nde tahminleri görelim!"
];

const nightMsgs = [
    "🌙 İyi geceler millet! Bırakın sistem uyurken de sizin için çalışsın.", 
    "🌌 Herkese iyi geceler. Yarın daha çok kazanacağız, sunucular soğutmaya alınıyor...", 
    "💤 Gece mesaisi bitenlere iyi uykular, yola devam edenlere bol kazançlar!",
    "🦉 İyi geceler Siber Kurtlar! Yatmadan önce günlük giriş (streak) ödülünü almayı unutma."
];

const randomMsgs = [
    "🚀 Piyasalar bugün hareketli. Sence BTC 10 saniye sonra nerede olacak?", 
    "💎 Unutmayın, panik yapan kaybeder, sabreden her zaman GEP kazanır.", 
    "🎁 Buralarda bir yerlerde yakında yeni bir Promo Kod düşebilir... Tetikte olun!",
    "⚔️ Cesareti olan yok mu? Yok mu birisine /duello atacak cengaver?"
];

setInterval(async () => {
    try {
        const s = await Settings.findOne();
        if (!s || !s.mainGroupId) return; // Ana grup seçilmediyse bir şey yapma
        ...
        // Türkiye Saati = UTC + 3
        // Sabah 09:00 TR => UTC 06:00
        if (utcHour === 6 && utcMin === 0) {
            const msg = morningMsgs[Math.floor(Math.random() * morningMsgs.length)];
            bot.sendMessage(s.mainGroupId, msg);
        }
        // Gece 23:30 TR => UTC 20:30
        else if (utcHour === 20 && utcMin === 30) {
            const msg = nightMsgs[Math.floor(Math.random() * nightMsgs.length)];
            bot.sendMessage(s.mainGroupId, msg);
        }
        // Rastgele Muhabbet: Öğleden sonra 15:00 TR => UTC 12:00
        else if (utcHour === 12 && utcMin === 0) {
            if (Math.random() > 0.3) { // %70 ihtimalle konuşsun (bazen sessiz kalır)
                const msg = randomMsgs[Math.floor(Math.random() * randomMsgs.length)];
                bot.sendMessage(s.mainGroupId, msg);
            }
        }
    } catch (e) { console.error("Otonom mesaj hatası:", e); }
}, 60000); // Her 1 dakikada bir saati kontrol eder

// Günlük Sıralama Arşivleme
...
app.post('/api/arcade/lootbox', secureRoute, async (req, res) => {
    ...
    if (!user || user.points < cost) return res.json({ success: false, message: "Yetersiz GEP!" });
    user.points -= cost; const rand = Math.random() * 100; let prize = 0; let msg = "BOŞ KAPSÜL";
    ...
    else if (boxType === 3) { if (rand <= 20) { prize = 0; msg = "🗑️ Çöp Veri"; } else if (rand <= 50) { prize = 15000; msg = "💎 Parazitli Node"; } else if (rand <= 80) { prize = 40000; msg = "🔥 Saf Çekirdek"; } else if (rand <= 95) { prize = 100000; msg = "👑 EFSANEVİ KOD 👑"; } else { prize = 500000; msg = "🌌 UZAY BOŞLUĞU 🌌"; } }
    if (prize > 0) addPoints(user, prize); await user.save(); res.json({ success: true, prize, msg, points: user.points });
});
...
// ==========================================
// 💬 TELEGRAM GRUP İÇİ KOMUTLAR (V5.0)
// ==========================================
...
// YENİ: GRUBU ANA MERKEZ OLARAK AYARLAMA (Sadece Admin)
bot.onText(/\/grububagla/, async (msg) => {
    if (String(msg.from.id) !== String(ADMIN_ID)) return;
    if (msg.chat.type === 'private') return bot.sendMessage(msg.chat.id, "Bu komut grupta yazılmalıdır.");
    
    const s = await Settings.findOne() || await Settings.create({});
    s.mainGroupId = msg.chat.id.toString();
    await s.save();
    
    bot.sendMessage(msg.chat.id, "✅ **Sistem Entegre Edildi!**\nArtık otonom mesajlar (Günaydın, İyi geceler) bu gruba düşecek.", { parse_mode: 'Markdown' });
});

bot.onText(/\/yardim/, (msg) => {
    const text = `🎮 **Gelir Evreni Grup Komutları**\n\n👤 \`/profil\` - Bakiyeni gösterir.\n💸 \`/bahsis <miktar>\` - (Yanıtla) GEP gönderir.\n🎲 \`/zar <miktar>\` - Zar atar, kazanırsan katlanır.\n🏆 \`/liderler\` - Top 5 oyuncuyu gösterir.\n⛏️ \`/maden\` - Üretim durumunu kontrol eder.\n⚔️ \`/duello <miktar>\` - Meydan okuma atar.`;
...
bot.onText(/\/zar (\d+)/, async (msg, match) => {
    const amount = parseInt(match[1]); const user = await User.findOne({ telegramId: msg.from.id.toString() });
    if (!user || user.points < amount || amount < 100) return bot.sendMessage(msg.chat.id, "❌ Yetersiz bakiye (Min 100 GEP).");
    user.points -= amount; await user.save();
    const diceMsg = await bot.sendDice(msg.chat.id); const value = diceMsg.dice.value;
    setTimeout(async () => {
        if (value >= 4) { const win = amount * 2; addPoints(user, win); await user.save(); bot.sendMessage(msg.chat.id, `🎉 **KAZANDIN!** Zar: ${value}\n💰 **+${win} GEP** hesabına eklendi!`, { reply_to_message_id: diceMsg.message_id }); } 
        else { bot.sendMessage(msg.chat.id, `💀 **KAYBETTİN...** Zar: ${value}\n💸 **${amount} GEP** havaya uçtu.`, { reply_to_message_id: diceMsg.message_id }); }
    }, 4000);
});
...
bot.onText(/\/maden/, async (msg) => {
    const user = await User.findOne({ telegramId: msg.from.id.toString() }); if (!user) return;
    const diff = new Date().getTime() - new Date(user.lastMining).getTime(); const cooldown = 4 * 60 * 60 * 1000;
    if (diff >= cooldown) { bot.sendMessage(msg.chat.id, "⛏️ **Madenin Hazır!**\nUygulamaya gir ve ödülünü topla. 🔥"); } 
    else { const remaining = Math.ceil((cooldown - diff) / (60 * 1000)); bot.sendMessage(msg.chat.id, `⛏️ Maden üretimde...\n⏳ **${remaining} dakika** sonra hazır.`); }
});
...
bot.onText(/\/duello (\d+)/, async (msg, match) => {
    ...
    if (!challenger || !opponent || challenger.points < amount || opponent.points < amount) { return bot.sendMessage(msg.chat.id, "❌ Düello için her iki tarafın da yeterli bakiyesi olmalı!"); }
...
bot.onText(/\/kabul/, async (msg) => {
    ...
    // Basitleştirilmiş rastgele düello sonucu
    bot.sendMessage(msg.chat.id, "⚔️ Kılıçlar çekildi! Sistem bir kazanan belirliyor...");
    setTimeout(() => { bot.sendMessage(msg.chat.id, `🎉 Düelloyu bitiren hamle geldi! Kazananın hesabı güncellendi.`); }, 3000);
});

bot.onText(/\/bahsis (\d+)/, async (msg, match) => {
    if (!msg.reply_to_message) return;
    const amount = parseInt(match[1]); const sender = await User.findOne({ telegramId: msg.from.id.toString() }); const receiver = await User.findOne({ telegramId: msg.reply_to_message.from.id.toString() });
    if (sender && receiver && sender.points >= amount && sender.telegramId !== receiver.telegramId) {
        sender.points -= amount; addPoints(receiver, amount); await sender.save(); await receiver.save();
